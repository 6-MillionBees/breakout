...
class Ball(pygame.sprite.Sprite):
  ...
  def check_pos(self) -> list[int]:
    """This checks whether the ball is within the window and then returns a list of numbers 1: left, 2: right, 3: up, 4: down. It also changes the direction."""
    collisions = []
  ...
  def colliderect(self, rect: pygame.Rect) -> int:
    """checks if the ball is colliding with a rectangle.
    Returns 0 if it isn't,
    returns 1 if it's a vertical collision,
    returns 2 if it's a horizontal collision,
    and returns 3 if it hits a corner"""
    ...
    if dist_y <= (rect.height / 2): return 1
    if dist_x <= (rect.width / 2): return 2
